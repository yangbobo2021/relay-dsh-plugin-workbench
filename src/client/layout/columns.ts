...
/** Side workbench panel floor for split-view contributions. */
export const SIDE_PANEL_MIN = 680
/** Side workbench panel ceiling for wide desktop layouts. */
export const SIDE_PANEL_MAX = 1120
/** Side workbench width before any user drag. */
export const SIDE_PANEL_DEFAULT = 820
/** Workbench bottom-panel drag clamp floor. */
export const BOTTOM_PANEL_MIN = 180
/** Workbench bottom-panel drag clamp ceiling. */
export const BOTTOM_PANEL_MAX = 520
/** Workbench bottom-panel height before any user drag. */
export const BOTTOM_PANEL_DEFAULT = 280
...
export function computeColumns(
  ...
  sidebar: number,
  details: number,
  range: { min: number; max: number; preserveMinimum?: boolean } = { min: DETAILS_MIN, max: DETAILS_MAX },
): Columns {
  ...
  const d1 = d0 === 0 ? 0 : Math.max(range.min, viewport - s - CENTER_MIN)
  if (s + d1 + CENTER_MIN <= viewport) return { sidebar: s, center: CENTER_MIN, details: d1 }

  // Split views need both sub-surfaces simultaneously. Preserve that workbench
  // minimum on desktop and let the conversation absorb the remaining squeeze.
  if (range.preserveMinimum === true && d0 > 0) {
    const preserved = Math.min(range.min, Math.max(0, viewport - s))
    return { sidebar: s, center: Math.max(0, viewport - s - preserved), details: preserved }
  }
